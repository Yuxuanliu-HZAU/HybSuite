...
# Cross-platform compatibility function for sed -i (Linux vs macOS)
sed_i() {
...
if [ "$#" -gt 0 ]; then
  case "$1" in
    ...
    "retrieve_results")
      ...
      if [ "$2" != "-i" ] && [ "$2" != "-o" ]; then
        echo -e "${red_color}[${timestamp}] [ERROR] You didn't specify the input directory (-i) or output directory (-o).${reset_color}"
        echo -e "${red_color}[${timestamp}] [ERROR] HybSuite exits.${reset_color}"
        echo ""
        exit 1 
      else
        if [ "$2" = "-i" ]; then
        ...
        if [ ! -d "${retrieve_input_dir}" ]; then
          echo -e "${red_color}[${timestamp}] [ERROR] The input directory does not exist.${reset_color}"
          echo -e "${red_color}[${timestamp}] [ERROR] HybSuite exits.${reset_color}"
          echo ""
          exit 1
        fi
        ...
        cp ${retrieve_input_dir}/08-Coalescent_analysis/*/05-PhyParts_PieCharts/*/*_phypartspiecharts_*.svg "${retrieve_output_dir}" 2>/dev/null || true
        cp ${retrieve_input_dir}/08-Coalescent_analysis/*/05-PhyParts_PieCharts/*/*_phypartspiecharts_*.tsv "${retrieve_output_dir}" 2>/dev/null || true
        exit 0
      fi
      ;;
      *)
...
#################===========================================================================
# Function: run_hybpiper_assemble
define_nt() {
...
define_threads() {
  ...
  local stage_log_file="${output_dir}/hybsuite_logs/hybsuite_${current_time}.log"
    ...
    if [ "$nt" -gt 0 ] && [ "${nt_software}" != "AUTO" ] 2>/dev/null; then
...
if [ "${check}" = "FALSE" ]; then
  ...
  if [ "${run_stage1}" = "TRUE" ] && [ "${full_pipeline}" = "FALSE" ]; then
    if [ ! -s "${input_list}" ]; then
      ...
      if [ "${found_pre}" = "1" ]; then
        if [ "${input_data}" = "_____" ]; then
          stage_error "The input data directory (specified by '-input_data') is not specified."
          stage_error "Please check and correct the '-input_data' parameter."
          stage_error "HybSuite exits."
          stage_blank_main ""
          exit 1
        fi
        species_list=$(awk -F'\t' '$2 == "B" { print $1 }' "${input_list}")
  ...
  if [ "${run_stage3}" = "TRUE" ] && [ "${full_pipeline}" = "FALSE" ]; then
    if [ ! -s "${input_list}" ]; then
      ...
      found_pre=$(awk -F'\t' '$2 == "B" { print 1; exit }' "${input_list}")
      if [ "${found_pre}" = "1" ] && ([ "${HRS}" = "TRUE" ] || [ "${RLWP}" = "TRUE" ]); then
        if [ "${input_data}" = "_____" ]; then
          stage_error "The input data directory (specified by '-input_data') is not specified."
          stage_error "Please check and correct the '-input_data' parameter."
          stage_error "HybSuite exits."
  ...
  if [ "${run_stage2}" = "TRUE" ] || [ "${run_stage3}" = "TRUE" ] || [ "${full_pipeline}" = "TRUE" ]; then
    ...
    if [ "${run_stage2}" = "TRUE" ] || [ "${full_pipeline}" = "TRUE" ]; then
      if [ "$(echo "${seqs_min_sample_coverage} >= 0" | bc -l)" -eq 1 ] && [ "$(echo "${seqs_min_sample_coverage} <= 1" | bc -l)" -eq 1 ]; then
        :
      else
        stage_error "The '-seqs_min_sample_coverage' value must be greater than 0 and less than 1."
        stage_error "Please correct it."
        stage_error "HybSuite exits."
        stage_blank_main ""
        exit 1
      fi
      if [ "$(echo "${seqs_min_locus_coverage} >= 0" | bc -l)" -eq 1 ] && [ "$(echo "${seqs_min_locus_coverage} <= 1" | bc -l)" -eq 1 ]; then
        :
      else
        stage_error "The '-seqs_min_locus_coverage' value must be greater than 0 and less than 1."
        stage_error "Please correct it."
        stage_error "HybSuite exits."
        stage_blank_main ""
        exit 1
    ...
    if [ "${run_stage3}" = "TRUE" ] || [ "${full_pipeline}" = "TRUE" ]; then
      ...
      if [ "$(echo "${aln_min_sample} >= 0" | bc -l)" -eq 1 ]; then
        :
      else
        stage_error "The '-aln_min_sample' value must be greater than 0."
        stage_error "Please correct it."
        stage_error "HybSuite exits."
        stage_blank_main ""
        exit 1
...
work_dir="${output_dir}/hybsuite_logs"
  ...
  display_only_failed_log() {
    ...
    # Display failed sample information
      if [ -s "$failed_log" ]; then
          failed_time=$(cat "${failed_log}.time" 2>/dev/null || echo "[$(date '+%Y-%m-%d %H:%M:%S')]")
          failed_samples=$(tr '\n' ' ' < "$failed_log")
          sed_i "/*\[LOG\] ${failing_prefix}*/d" "$log_file"
  ...
  replace_n() {
    local input_fasta=$1
    sed -e '/^>/! s/n/-/g' -e '/^>/! s/N/-/g' "$input_fasta" > "${input_fasta}.tmp" &&
...
run_hmmcleaner() {
    ...
    if (( $(echo "${cost1} < 0 && ${cost2} < 0 && ${cost3} > 0 && ${cost4} > 0 && \
        ...
        if [ "${seq_num}" -lt 2 ]; then
            ...
            stage_cmd "${log_mode}" "${dependencies_dir}/Bio-MUST-Apps-HmmCleaner-0.243280/bin/HmmCleaner.pl ${input_file} -costs ${cost1} ${cost2} ${cost3} ${cost4} --noX"
            ...
            ${dependencies_dir}/Bio-MUST-Apps-HmmCleaner-0.243280/bin/HmmCleaner.pl "${input_file}" -costs "${cost1}" "${cost2}" "${cost3}" "${cost4}" --noX 2>&1 >/dev/null | grep -v "WARNING\|at reader Bio\|line"
        else
            stage_cmd "${log_mode}" "${dependencies_dir}/Bio-MUST-Apps-HmmCleaner-0.243280/bin/HmmCleaner.pl ${input_file} -costs ${cost1} ${cost2} ${cost3} ${cost4} --noX --large"
            ...
            ${dependencies_dir}/Bio-MUST-Apps-HmmCleaner-0.243280/bin/HmmCleaner.pl "${input_file}" -costs "${cost1}" "${cost2}" "${cost3}" "${cost4}" --noX --large 2>&1 >/dev/null | grep -v "WARNING\|at reader Bio\|line"
        fi
    else
        stage_blank_main ""
...
if ([ "${run_stage1}" = "TRUE" ] || [ "${full_pipeline}" = "TRUE" ]) && [ "${skip_stage1}" = "FALSE" ]; then
  ...
  if [ -s "${output_dir}/hybsuite_checklists/Public_Spname_SRR.txt" ]; then
      ...
      stage_info_main "====>> Running sratoolkit to download raw data (${process} in parallel; ${nt_pigz} threads for pigz; ${nt_fasterq_dump} threads for fasterq-dump) ====>>"
      while IFS= read -r sample || [ -n "$sample" ]; do
  ...
  if [ -s "${output_dir}/hybsuite_checklists/Public_Spname_SRR.txt" ]; then
      ...
      # Initialize parallel environment
      init_parallel_env "$work_dir" "$total_sps" "$process" "${output_dir}/hybsuite_checklists/Public_Spname_SRR.txt" || exit 1
      stage_info_main "====>> Removing adapters for downloaded raw data of ${total_sps} samples (${process} in parallel; ${nt_trimmomatic} threads) ====>>"
  ...
  if [ -s "${output_dir}/hybsuite_checklists/My_Spname.txt" ] && [ -e "${input_data}" ]; then
    ...
    stage_info_main "====>> Removing adapters for existing (user-provided) raw data of ${total_sps} samples (${process} in parallel; ${nt_trimmomatic} threads) ====>>"
    stage_info_main "Sequences that have already had adapters removed will not be trimmed for adapters again!!!"
    while IFS= read -r sample || [ -n "$sample" ]; do
      if [ "${process}" != "all" ]; then
          read -u1000
  ...
  stage_info_main_success "Output directorys: ${NGS_dir}"
  ...
  if [ "${full_pipeline}" = "FALSE" ]; then
    ...
    stage_blank_main ""
    exit 0
  elif [ "${full_pipeline}" = "TRUE" ] && [ "${run_until}" = "FALSE" ]; then
...
if ([ "${run_stage2}" = "TRUE" ] || [ "${full_pipeline}" = "TRUE" ]) && [ "${skip_stage2}" = "FALSE" ]; then
  ...
  #################===========================================================================
  #Function: run_hybpiper_assemble
  run_hybpiper_assemble() {
    local input_directory=$1
    local Spname=$2

    # For paired-end
    if ([ -s "${input_directory}/${Spname}_1_clean.paired.fq.gz" ] && [ -s "${input_directory}/${Spname}_2_clean.paired.fq.gz" ]) \
    && [ ! -s "${input_directory}/${Spname}_clean.single.fq.gz" ]; then
      hybpiper_cmd="hybpiper assemble -t_${hybpiper_tt} ${t} -r ${input_directory}/${Spname}_1_clean.paired.fq.gz ${input_directory}/${Spname}_2_clean.paired.fq.gz -o ${eas_dir} --prefix ${Spname} --cpu ${nt_hybpiper}"
      # 01: The type of the target file
      # for protein reference sequence
      if [ "${hybpiper_tt}" = "aa" ] && [ "${hybpiper_mapping_tool}" = "diamond" ]; then
        hybpiper_cmd="${hybpiper_cmd} --diamond"
      fi
      # for nucleotide reference sequence
      if [ "${hybpiper_tt}" = "dna" ]; then
        hybpiper_cmd="${hybpiper_cmd} --bwa"
      fi
    ...
    if ([ ! -s "${input_directory}/${Spname}_1_clean.paired.fq.gz" ] && [ ! -s "${input_directory}/${Spname}_2_clean.paired.fq.gz" ]) \
    && [ -s "${input_directory}/${Spname}_clean.single.fq.gz" ]; then
      hybpiper_cmd="hybpiper assemble -t_${hybpiper_tt} ${t} -r ${input_directory}/${Spname}_clean.single.fq.gz -o ${eas_dir} --prefix ${Spname} --cpu ${nt_hybpiper}"
      # 01: The type of the target file
      # for protein reference sequence
      if [ "${hybpiper_tt}" = "aa" ] && [ "${hybpiper_mapping_tool}" = "diamond" ]; then
        hybpiper_cmd="${hybpiper_cmd} --diamond"
    ...
    eval "${hybpiper_cmd}" > /dev/null 2>&1
  }
  #################===========================================================================
  ...
  if [ "${found_public}" = "1" ]; then
    ...
    stage_info_main "====>> Running 'hybpiper assemble' to assemble public data (${process} in parallel; ${nt_hybpiper} threads) ====>>"
    while IFS= read -r Spname || [ -n "$Spname" ]; do
      ...
      fi
      {
        ...
        run_hybpiper_assemble "${NGS_dir}/02-Downloaded_clean_data" "${Spname}"
        if [ -s "${eas_dir}/${Spname}/genes_with_seqs.txt" ]; then
            update_finish_count "$Spname" "$stage_logfile"
        else
            record_failed_sample "$Spname"
  ...
  if [ -s "${output_dir}/hybsuite_checklists/My_Spname.txt" ]; then
    ...
    stage_info_main "====>> Running 'hybpiper assemble' to assemble user-provided data (${process} in parallel; ${nt_hybpiper} threads) ====>>"
    while IFS= read -r Spname || [ -n "$Spname" ]; do
      if [ "${process}" != "all" ]; then
          read -u1000
      fi
      ...
      fi
      {
        ...
        run_hybpiper_assemble "${NGS_dir}/03-My_clean_data" "${Spname}"
        if [ -s "${eas_dir}/${Spname}/genes_with_seqs.txt" ]; then
            update_finish_count "$Spname" "$stage_logfile"
        else
            record_failed_sample "$Spname"
  ...
  #################===========================================================================
    stage_info_main_blue "Step 2: retrieving all paralog sequences via HybPiper..."
  ...
  ############################################################################################
  #Stage2-Optional step: Integrating pre-assembled sequences into all paralogs ###############
  ############################################################################################
    if [ "${found_pre}" != "1" ]; then
      ...
      stage_info_main "Optional step: Integrating pre-assembled sequences into all paralogs ..."
      # 01-Integrating pre-assembled sequences into all paralogs
      stage_cmd_main "python ${script_dir}/integrate_seqs.py -i ${input_data} -o ${output_dir}/02-All_paralogs/01-Original_paralogs -s _paralogs_all.fasta --locus_list ${output_dir}/hybsuite_checklists/Ref_gene_name_list.txt --sample_list ${output_dir}/hybsuite_checklists/Pre-assembled_Spname.txt -l ${stage_logfile}"
      python "${script_dir}/integrate_seqs.py" -i "${input_data}" -o "${output_dir}/02-All_paralogs/01-Original_paralogs" -s "_paralogs_all.fasta" --locus_list "${output_dir}/hybsuite_checklists/Ref_gene_name_list.txt" --sample_list "${output_dir}/hybsuite_checklists/Pre-assembled_Spname.txt" -l "${stage_logfile}"
      # 02-Formatting the output fasta files
  ...
  #################===========================================================================
    ...
    stage_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_sample_and_locus_coverage.py -i ${output_dir}/02-All_paralogs/03-Filtered_paralogs --min_locus_coverage ${seqs_min_locus_coverage} --min_sample_coverage ${seqs_min_sample_coverage} --removed_samples_info ${output_dir}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Removed_samples_with_low_locus_coverage_info.tsv --removed_loci_info ${output_dir}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Removed_loci_with_low_sample_coverage_info.tsv --threads ${nt}"
    stage2_02="python ${script_dir}/filter_seqs_by_sample_and_locus_coverage.py \
    -i ${output_dir}/02-All_paralogs/03-Filtered_paralogs \
    --min_locus_coverage ${seqs_min_locus_coverage} \
    --min_sample_coverage ${seqs_min_sample_coverage} \
    ...
    if [ -s "${output_dir}/02-All_paralogs/02-Original_paralog_reports_and_heatmap/Original_paralog_heatmap.png" ]; then
      ...
      stage_warning "Fail to produce the paralog heatmap for the original paralogs."
    fi
    if [ -s "${output_dir}/02-All_paralogs/02-Original_paralog_reports_and_heatmap/Original_paralog_report.tsv" ]; then
      ...
      stage_warning "Fail to produce the paralog report for the original paralogs."
    fi

    # 02-Producing the paralog report and plotting the paralog heatmap for the filtered paralogs
    stage_info_main "02-Producing the paralog report and plotting the paralog heatmap for the filtered paralogs..."
    ...
    if [ -s "${output_dir}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_paralog_heatmap.png" ]; then
      ...
      stage_warning "Fail to produce the paralog heatmap for the filtered paralogs."
    fi
    if [ -s "${output_dir}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_paralog_report.tsv" ]; then
      ...
      stage_warning "Fail to produce the paralog report for the filtered paralogs."
    fi
    stage_blank_main ""
  ...
  stage_info_main_success "Output directory 1: ${output_dir}/01-Assembled_data/"  
  stage_info_main_success "(1) HybPiper assembled data directory: ${output_dir}/01-Assembled_data/<sample_name>"
  stage_info_main_success "Output directory 2: ${output_dir}/02-All_paralogs/"
...
if ([ "${run_stage3}" = "TRUE" ] || [ "${full_pipeline}" = "TRUE" ]) && [ "${skip_stage3}" = "FALSE" ]; then
  ...
  # 0.Preparation
  stage_blank_main ""
  stage_info_main_purple "<<<======= Stage 3 Paralog handling =======>>>"
  if [ "${full_pipeline}" = "TRUE" ]; then
  ...
  retrieve_hybpiper_sequences() {
    ...
    mkdir -p "${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/"
    hybpiper_cmd="hybpiper retrieve_sequences -t_${hybpiper_tt} ${t} ${hybpiper_retrieved_seqs_type} --hybpiper_dir ${eas_dir} --sample_names ${eas_dir}/Assembled_data_namelist.txt --fasta_dir ${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/"
    if [ "${hybpiper_skip_chimeric_genes}" = "TRUE" ]; then
      hybpiper_cmd="${hybpiper_cmd} --skip_chimeric_genes"
    fi
    stage_cmd_main "${hybpiper_cmd}"
    eval "${hybpiper_cmd}" > /dev/null 2>&1
    if find "${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/" -type f -name "*.FNA" -size +0c -quit 2>/dev/null; then
  ...
  filter_seqs_by_llsc() {
    ...
    mkdir -p "${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/" "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/"
    # 01-Removing ${ortho_method} sequences with low length
    stage_info_main "01-Removing ${ortho_method} sequences with low length..."
    stage_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_length.py -i ${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/ -or ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_${ortho_method}_seqs_with_low_length_info.tsv --output_dir ${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/ --ref ${t} --min_length ${seqs_min_length} --mean_length_ratio ${seqs_mean_length_ratio} --max_length_ratio ${seqs_max_length_ratio} --threads ${nt}"
    stage3_01="python ${script_dir}/filter_seqs_by_length.py \
    -i ${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/ \
    -or ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_${ortho_method}_seqs_with_low_length_info.tsv \
    --output_dir ${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/ \
    --ref ${t} \
    --min_length ${seqs_min_length} \
    --mean_length_ratio ${seqs_mean_length_ratio} \
    --max_length_ratio ${seqs_max_length_ratio} \
    ...
    if ! find "${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/" -maxdepth 1 -type f -name "*.FNA" | grep -q .; then
      stage_error "Failed to Remove ${ortho_method} sequences with length than ${seqs_min_length}."
      stage_error "HybSuite exits."
      stage_blank_main ""
      exit 1
    else
      if [ -s "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_${ortho_method}_seqs_with_low_length_info.tsv" ]; then
        stage_info_main "The information of removed ${ortho_method} sequences with low length has been written to:"
        stage_info_main "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_${ortho_method}_seqs_with_low_length_info.tsv"
      else
    ...
    stage_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_sample_and_locus_coverage.py -i ${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/ --min_locus_coverage ${seqs_min_locus_coverage} --min_sample_coverage ${seqs_min_sample_coverage} --removed_samples_info ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_samples_with_low_locus_coverage_info.tsv --removed_loci_info ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_loci_with_low_sample_coverage_info.tsv --threads ${nt}"
    stage3_02="python ${script_dir}/filter_seqs_by_sample_and_locus_coverage.py \
    -i ${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/ \
    --min_locus_coverage ${seqs_min_locus_coverage} \
    --min_sample_coverage ${seqs_min_sample_coverage} \
    --removed_samples_info ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_samples_with_low_locus_coverage_info.tsv \
    --removed_loci_info ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_loci_with_low_sample_coverage_info.tsv \
    --threads ${nt}"
    if [ "${log_mode}" = "full" ]; then
      eval "${stage3_02}"
    else
    ...
    if ! find "${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/" -maxdepth 1 -type f -name "*.FNA" | grep -q .; then
      ...
      if [ -s "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_samples_with_low_locus_coverage_info.tsv" ]; then
        stage_info_main "The information of removed taxa with low locus coverage has been written to:"
        stage_info_main "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_samples_with_low_locus_coverage_info.tsv"
      else
        stage_info_main "No sample was removed."
      fi
      if [ -s "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_loci_with_low_sample_coverage_info.tsv" ]; then
        stage_info_main "The information of removed loci with low sample coverage has been written to:"
        stage_info_main "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Removed_loci_with_low_sample_coverage_info.tsv"
      else
  ...
  MSA_and_trim() {
    ...
    while IFS= read -r file || [ -n "$file" ]; do
      ...
      if [ ! -s "${output_dir}/04-Alignments/${ortho_method}/${file_name}.aln.fasta" ]; then
        ...
        # Update finish count
        sed_i 's/>_R_/>/g' "${output_dir}/04-Alignments/${ortho_method}/${file_name}.aln.fasta"
    ...
    if [ "${trim_tool}" = "1" ]; then
      ...
      stage_info_main "====>> Trimming ${ortho_method} alignments via trimAl (${process} in parallel; ${nt_trimal} threads) ====>>"
    elif [ "${trim_tool}" = "2" ]; then
      stage_info_main "02-Trimming ${ortho_method} alignments via HMMCleaner..."
      stage_info_main "====>> Trimming ${ortho_method} alignments via HMMCleaner (${process} in parallel; ${nt_hmmcleaner} threads) ====>>"
    fi
  ...
  generate_reports_heatmap() {
    ...
    rm -rf "${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/"
    mkdir -p "${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/"
    ...
    stage_cmd "${log_mode}" "python ${script_dir}/plot_recovery_heatmap.py -i ${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/ -r ${t} -osl ${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/Original_${ortho_method}_seq_lengths.tsv -oh ${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/Original_${ortho_method}_heatmap --threads ${nt} --color ${heatmap_color}"
    stage3_03="python ${script_dir}/plot_recovery_heatmap.py \
    -i ${output_dir}/03-Paralog_handling/${ortho_method}/01-Original_${ortho_method}_sequences/ \
    -r ${t} \
    -osl ${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/Original_${ortho_method}_seq_lengths.tsv \
    -oh ${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/Original_${ortho_method}_heatmap \
    --threads ${nt} \
    --color ${heatmap_color}"
    if [ "${log_mode}" = "full" ]; then
      eval "${stage3_03}"
    ...
    if [ -s "${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/Original_${ortho_method}_heatmap.png" ]; then
      stage_info "${log_mode}" "The reports and heatmap of the original ${ortho_method} sequences have been written to:"
      stage_info "${log_mode}" "${output_dir}/03-Paralog_handling/${ortho_method}/02-Original_${ortho_method}_sequences_reports_and_heatmap/"
    else
      stage_warning "Failed to get the reports and heatmap of the original ${ortho_method} sequences."
    fi
    ...
    stage_cmd_main "python ${script_dir}/plot_recovery_heatmap.py -i ${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/ -r ${t} -osl ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Filtered_${ortho_method}_seq_lengths.tsv -oh ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Filtered_${ortho_method}_heatmap --threads ${nt} --show_values --color ${heatmap_color}"
    stage3_04="python ${script_dir}/plot_recovery_heatmap.py \
    -i ${output_dir}/03-Paralog_handling/${ortho_method}/03-Filtered_${ortho_method}_sequences/ \
    -r ${t} \
    -osl ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Filtered_${ortho_method}_seq_lengths.tsv \
    -oh ${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Filtered_${ortho_method}_heatmap \
    --threads ${nt} \
    --color ${heatmap_color}"
    if [ "${log_mode}" = "full" ]; then
      eval "${stage3_04}"
    ...
    if [ -s "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/Filtered_${ortho_method}_heatmap.png" ]; then
      stage_info "${log_mode}" "The reports and heatmap of the filtered ${ortho_method} sequences have been written to:"
      stage_info "${log_mode}" "${output_dir}/03-Paralog_handling/${ortho_method}/04-Filtered_${ortho_method}_sequences_reports_and_heatmap/"
      stage_success "Finished."
      stage_blank_main ""
    else
  ...
  summarize_and_filter_alignments() {
    ...
    sed_i '1d' "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-03_Removed_alignments_without_parsimony_informative_sites.txt
    ...
    sed_i '1d' "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-04_Removed_alignments_with_length_less_than_"${aln_min_length}".txt
    ...
    # 05_Removed_alignments_with_sample_number_less_than_${aln_min_sample}.txt
    count_species "${output_dir}/05-Trimmed_alignments/${ortho_method}"
    awk -v cov_thresh="${aln_min_sample}" -v cov_length_thresh="${aln_min_length}" \
    '$9 != 0 && $3 >= cov_length_thresh && $2 < cov_thresh {print $1}' \
    "${output_dir}/hybsuite_reports/Alignments_stats/${ortho_method}-02_Trimmed_alignments_stats_AMAS.tsv" \
    > "${output_dir}/hybsuite_reports/Alignments_stats/${ortho_method}-05_Removed_alignments_with_sample_number_less_than_${aln_min_sample}.txt"
    sed_i '1d' "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-05_Removed_alignments_with_sample_number_less_than_${aln_min_sample}.txt
    num_filtered_s=$(wc -l < "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-05_Removed_alignments_with_sample_number_less_than_${aln_min_sample}.txt)
    stage_info_main "Removed ${num_filtered_s} ${ortho_method} alignments with sample number less than ${aln_min_sample}."

    # 06_Final_alignments_list.txt
    awk -v cov_thresh="${aln_min_sample}" -v cov_length_thresh="${aln_min_length}" '$9!=0 && $3 >= cov_length_thresh && $2 >= cov_thresh {print $1}' "${output_dir}/hybsuite_reports/Alignments_stats/${ortho_method}-02_Trimmed_alignments_stats_AMAS.tsv" > "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-06_Final_alignments_list.txt
    sed_i '1d' "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-06_Final_alignments_list.txt
    if [ ! -s "${output_dir}/hybsuite_reports/Alignments_stats/${ortho_method}-06_Final_alignments_list.txt" ]; then
      ...
      stage_error "Please adjust the parameter '-aln_min_length' and '-aln_min_sample' to run hybsuite again."
      stage_error "HybSuite exits."
      stage_blank_main ""
      exit 1
    fi
    ...
    awk -v cov_thresh="${aln_min_sample}" -v cov_length_thresh="${aln_min_length}" '$9!=0 && $3 >= cov_length_thresh && $2 >= cov_thresh {print $0}' "${output_dir}/hybsuite_reports/Alignments_stats/${ortho_method}-02_Trimmed_alignments_stats_AMAS.tsv" > "${output_dir}"/hybsuite_reports/Alignments_stats/${ortho_method}-07_Final_alignments_stats_AMAS.tsv
    rm -rf "${output_dir}/06-Final_alignments/${ortho_method}/"
  ...
  if [ "${HRS}" = "TRUE" ]; then
    ...
    if [ -d "${output_dir}/03-Paralog_handling/HRS/" ]; then
      rm -rf "${output_dir}/03-Paralog_handling/HRS/"
    fi
    mkdir -p "${output_dir}/03-Paralog_handling/HRS/"
    #################===========================================================================
    ...
    if [ "${found_pre}" != "1" ]; then
      ...
      # 01-Integrating pre-assembled sequences into HRS dataset
      stage_cmd_main "python ${script_dir}/integrate_seqs.py -i ${input_data} -o ${output_dir}/03-Paralog_handling/HRS/01-Original_HRS_sequences -s .FNA --locus_list ${output_dir}/hybsuite_checklists/Ref_gene_name_list.txt --sample_list ${output_dir}/hybsuite_checklists/Pre-assembled_Spname.txt -l ${stage_logfile} --single_copy"
      python "${script_dir}/integrate_seqs.py" -i "${input_data}" -o "${output_dir}/03-Paralog_handling/HRS/01-Original_HRS_sequences" -s ".FNA" --locus_list "${output_dir}/hybsuite_checklists/Ref_gene_name_list.txt" --sample_list "${output_dir}/hybsuite_checklists/Pre-assembled_Spname.txt" -l "${stage_logfile}" --single_copy
      # 02-Formatting the output fasta files
      cd "${output_dir}"/03-Paralog_handling/HRS/01-Original_HRS_sequences
      for file in *.FNA; do
        python "${script_dir}/Fasta_formatter.py" -i "${file}" -o "${file}" -nt "${nt}" --inter > /dev/null 2>&1
      done
    fi
  ...
  if [ "${RLWP}" = "TRUE" ]; then
    ...
    if [ -d "${output_dir}/03-Paralog_handling/RLWP/" ]; then
      rm -rf "${output_dir}/03-Paralog_handling/RLWP/"
    fi
    mkdir -p "${output_dir}/03-Paralog_handling/RLWP/"
    #################===========================================================================
    ...
    if [ "${found_pre}" != "1" ]; then
      ...
      stage_cmd_main "python ${script_dir}/integrate_seqs.py -i ${input_data} -o ${output_dir}/03-Paralog_handling/RLWP/01-Original_RLWP_sequences -s .FNA --locus_list ${output_dir}/hybsuite_checklists/Ref_gene_name_list.txt --sample_list ${output_dir}/hybsuite_checklists/Pre-assembled_Spname.txt -l ${stage_logfile} --single_copy"
      python "${script_dir}/integrate_seqs.py" -i "${input_data}" -o "${output_dir}/03-Paralog_handling/RLWP/01-Original_RLWP_sequences" -s ".FNA" --locus_list "${output_dir}/hybsuite_checklists/Ref_gene_name_list.txt" --sample_list "${output_dir}/hybsuite_checklists/Pre-assembled_Spname.txt" -l "${stage_logfile}" --single_copy
      # 02-Formatting the output fasta files
      cd "${output_dir}"/03-Paralog_handling/RLWP/01-Original_RLWP_sequences
      for file in *.FNA; do
        python "${script_dir}/Fasta_formatter.py" -i "${file}" -o "${file}" -nt "${nt}" --inter > /dev/null 2>&1
      done
    fi
    ...
    #################===========================================================================
    mkdir -p "${output_dir}/03-Paralog_handling/RLWP/02-Original_RLWP_sequences_reports_and_heatmap/"
    stage_cmd "${log_mode}" "python ${script_dir}/RLWP.py -i ${output_dir}/03-Paralog_handling/RLWP/01-Original_RLWP_sequences/ -p ${output_dir}/02-All_paralogs/02-Original_paralog_reports_and_heatmap/Original_paralog_report.tsv -s ${rlwp} -or ${output_dir}/03-Paralog_handling/RLWP/02-Original_RLWP_sequences_reports_and_heatmap/RLWP_removed_loci_info.tsv --threads ${nt}"
    stage3_RLWP_1="python ${script_dir}/RLWP.py \
    -i ${output_dir}/03-Paralog_handling/RLWP/01-Original_RLWP_sequences/ \
    -p ${output_dir}/02-All_paralogs/02-Original_paralog_reports_and_heatmap/Original_paralog_report.tsv \
    -s ${rlwp} \
    -or ${output_dir}/03-Paralog_handling/RLWP/02-Original_RLWP_sequences_reports_and_heatmap/RLWP_removed_loci_info.tsv \
    --threads ${nt}"
    if [ "${log_mode}" = "full" ]; then
      eval "${stage3_RLWP_1}"
    else
  ...
  if [ "$LS" = "TRUE" ] || { ([ "$MO" = "TRUE" ] || [ "$MI" = "TRUE" ] || [ "$RT" = "TRUE" ] || [ "${one_to_one}" = "TRUE" ]) && [ "${run_phylopypruner}" = "TRUE" ] && [ "${run_paragone}" != "TRUE" ]; }; then
    ...
    #################===========================================================================
    if [ -d "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input" ]; then
      rm -r "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input"
    fi
    ...
    stage_info_main "====>> Running MAFFT, ${trim_tool_tool} and ${gene_tree_tool} for ${total_sps} all paralog files (${process} in parallel ====>>"
    ...
    while IFS= read -r file || [ -n "$file" ]; do
      ...
      fi
      {
        ...
        sed_i "s/ single_hit/|single_hit/g;s/ multi/|multi/g;s/ NODE_/|NODE_/g;s/\.[0-9]\+|NODE_/|NODE_/g;s/\.main|NODE_/|NODE_/g" "${file}"
        # Run MAFFT  
        run_mafft "${file}" "${filename}.aln.fasta" "${nt_mafft}"
        remove_n "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.fasta"
        sed_i 's/>_R_/>/g' "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.fasta"
        if [ "${replace_n}" = "TRUE" ]; then
          replace_n "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.fasta"
        fi
        if [ "${trim_tool}" = "1" ]; then
          run_trimal "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.fasta" "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta" "${trimal_mode}" \
          "${trimal_gapthreshold}" "${trimal_simthreshold}" "${trimal_cons}" "${trimal_block}" "${trimal_resoverlap}" "${trimal_seqoverlap}" \
          "${trimal_w}" "${trimal_gw}" "${trimal_sw}"
        fi
        if [ "${trim_tool}" = "2" ]; then
          run_hmmcleaner "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.fasta" "${hmmcleaner_cost}"
          if [ -s "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln_hmm.fasta" ]; then
            update_finish_count "$genename" "$stage_logfile"
            mv "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln_hmm.fasta" "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta"
            rm -f "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln_hmm"*
          else
        ...
        { print $0 }' "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta" > "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.temp" && \
        mv "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.temp" "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta"
        if [ ! -s "./${filename}.aln.trimmed.fasta" ]; then
          record_failed_sample "$genename"
        fi
        if [ "${gene_tree}" = "1" ]; then
          stage_cmd "${log_mode}" "iqtree -s ${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta -m MFP -nt ${nt_iqtree} -bb ${gene_tree_bb} -pre ${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta"
          iqtree -s "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta" -m MFP -nt ${nt_iqtree} -bb "${gene_tree_bb}" -pre "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta" > /dev/null 2>&1
          if [ -s "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta.treefile" ]; then
            mv "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta.treefile" "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta.tre"
            find . -maxdepth 1 -name "${filename}.aln.trimmed.fasta*" ! -name "*.tre" ! -name "*.fasta" -delete
          else
            record_failed_sample "$genename"
          fi
        elif [ "${gene_tree}" = "2" ]; then
          stage_cmd "${log_mode}" "FastTreeMP -nt -gtr -gamma -boot ${gene_tree_bb} ${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta > ./${filename}.aln.trimmed.fasta.tre"
          FastTreeMP -nt -gtr -gamma -boot "${gene_tree_bb}" "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta" > "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta.tre" 2>/dev/null
          if [ ! -s "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input/${filename}.aln.trimmed.fasta.tre" ]; then
            record_failed_sample "$genename"
          fi
        fi
    ...
    run_phylopypruner() {
        ...
        for file in ${input_dir}/PhyloPyPruner/${output_phylopypruner_dir}/output_alignments/*_paralogs_all.aln.trimmed_pruned*.fasta; do
          gene_name="${file%%_paralogs_all*}"
          middle_part="${file#*_paralogs_all.aln.trimmed_pruned}"
          middle_part="${middle_part%.fasta}"
          new_file="${gene_name}${middle_part}_paralogs_all.aln.trimmed.pruned.fasta"
          mv "$file" "$new_file"
        done
    ...
    if [ "${LS}" = "TRUE" ]; then
        run_phylopypruner "LS" "Output_LS" "${nt_phylopypruner}" "${output_dir}/03-Paralog_handling" "${pp_trim_lb}" "${pp_min_taxa}"
    fi

    # Run PhyloPyPruner for MI
    if [ "${MI}" = "TRUE" ] && [ "${run_paragone}" = "FALSE" ]; then
        run_phylopypruner "MI" "Output_MI" "${nt_phylopypruner}" "${output_dir}/03-Paralog_handling" "${pp_trim_lb}" "${pp_min_taxa}"
    fi

    # Run PhyloPyPruner for MO
    if [ "${MO}" = "TRUE" ] && [ "${run_paragone}" = "FALSE" ]; then
        run_phylopypruner "MO" "Output_MO" "${nt_phylopypruner}" "${output_dir}/03-Paralog_handling" "${pp_trim_lb}" "${pp_min_taxa}" "${output_dir}/hybsuite_checklists/Outgroup.txt"
    fi

    # Run PhyloPyPruner for RT
    if [ "${RT}" = "TRUE" ] && [ "${run_paragone}" = "FALSE" ]; then
        run_phylopypruner "RT" "Output_RT" "${nt_phylopypruner}" "${output_dir}/03-Paralog_handling" "${pp_trim_lb}" "${pp_min_taxa}" "${output_dir}/hybsuite_checklists/Outgroup.txt"
    fi
    
    # Run PhyloPyPruner for 1to1
    if [ "${one_to_one}" = "TRUE" ] && [ "${run_paragone}" = "FALSE" ]; then
        run_phylopypruner "1to1" "Output_1to1" "${nt_phylopypruner}" "${output_dir}/03-Paralog_handling" "${pp_trim_lb}" "${pp_min_taxa}"
    fi
  ...
  if ([ "$MO" = "TRUE" ] || [ "$MI" = "TRUE" ] || [ "$RT" = "TRUE" ] || [ "${one_to_one}" = "TRUE" ]) && [ "${run_paragone}" = "TRUE" ]; then
    ...
    run_paragone() {
      ...
      if [ "${MO}" = "TRUE" ]; then
        ...
        sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g" "${output_dir}/03-Paralog_handling/ParaGone/23_MO_final_alignments/"*
        sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g" "${output_dir}/03-Paralog_handling/ParaGone/26_MO_final_alignments_trimmed/"*
        cp "${output_dir}/03-Paralog_handling/ParaGone/23_MO_final_alignments"/*.fasta "${output_dir}/04-Alignments/MO/"
        cp "${output_dir}/03-Paralog_handling/ParaGone/26_MO_final_alignments_trimmed"/*.fasta "${output_dir}/05-Trimmed_alignments/MO/"
      fi
      if [ "${MI}" = "TRUE" ]; then
        stage_info_main "====>> MI ====>>"
        ...
        sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g" "${output_dir}/03-Paralog_handling/ParaGone/24_MI_final_alignments/"*
        sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g" "${output_dir}/03-Paralog_handling/ParaGone/27_MI_final_alignments_trimmed/"*
        cp "${output_dir}/03-Paralog_handling/ParaGone/24_MI_final_alignments"/*.fasta "${output_dir}/04-Alignments/MI/"
        cp "${output_dir}/03-Paralog_handling/ParaGone/27_MI_final_alignments_trimmed"/*.fasta "${output_dir}/05-Trimmed_alignments/MI/"
      fi
      if [ "${RT}" = "TRUE" ]; then
        stage_info_main "====>> RT ====>>"
        ...
        sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g" "${output_dir}/03-Paralog_handling/ParaGone/28_RT_final_alignments_trimmed/"*
        cp "${output_dir}/03-Paralog_handling/ParaGone/28_RT_final_alignments_trimmed"/*.fasta "${output_dir}/05-Trimmed_alignments/RT/"
      fi
      if [ "${one_to_one}" = "TRUE" ]; then
        stage_info_main "====>> 1to1 ====>>"
        # Orthogroup alignments
        cd "${output_dir}/03-Paralog_handling/ParaGone/23_MO_final_alignments"
        ...
        for file in $files; do
          ...
          sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g"  "${output_dir}/04-Alignments/1to1/${file}"
        ...
        cd "${output_dir}/03-Paralog_handling/ParaGone/26_MO_final_alignments_trimmed"
        ...
        for file in $files; do
          ...
          sed_i "s/_var_/_var\._/g;s/_subsp_/_subsp\._/g;s/_f_/_f\._/g"  "${output_dir}/05-Trimmed_alignments/1to1/${file}"
          progress=$((j * 100 / total_sps))
  ...
  ############################################################################################
  # End of Stage 3
  stage_success "Successfully finished Stage3: 'Paralog handling'."
  stage_info_main_success "The resulting files have been saved in:"
  stage_info_main_success "(1) ${output_dir}/03-Paralog_handling/"
...
gene_tree_iqtree_fasttree() {
    ...
    if [ "${gene_tree}" = "1" ]; then
      gene_tree_tool="IQ-TREE"
      define_threads "iqtree"
      stage_info_main "====>> Running ${gene_tree_tool} to construct single gene trees (${process} in parallel; ${nt_iqtree} threads for IQ-TREE) ====>>"
    elif [ "${gene_tree}" = "2" ]; then
      gene_tree_tool="FastTree"
      define_threads "fasttree"
      export OMP_NUM_THREADS=${nt_fasttree}
...
if [ "${run_astral_pro}" = "TRUE" ]; then
  ...
  if find "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input" -type f -name '*.tre' -size +0c -quit 2>/dev/null; then
    #################===========================================================================
    ...
    cp "${output_dir}/03-Paralog_handling/PhyloPyPruner/Input"/*.tre "${output_dir}/08-Coalescent_analysis/ASTRAL-Pro/01-Gene_trees/"
    sed_i -E 's/\|[^:]*:/:/g; s/\|//g' "${output_dir}/08-Coalescent_analysis/ASTRAL-Pro/01-Gene_trees/"*.tre
    ...
    while IFS= read -r file || [ -n "$file" ]; do
      ...
      fi
      {
        ...
        sed_i 's/>_R_/>/g' "${output_dir}/08-Coalescent_analysis/ASTRAL-Pro/01-Gene_trees/${filename}.aln.fasta"
