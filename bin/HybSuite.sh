...
if [ "${skip_checking}" != "TRUE" ]; then
  ...
  if (( $(echo "${min_taxon_coverage} >= 0" | bc -l) )) && (( $(echo "${min_taxon_coverage} <= 1" | bc -l) )); then
    sleep 1
  else
    stage0_info "Verifing -min_taxon_coverage..."
    stage0_info "Validation result:"
    stage0_error "The -taxon_coverage value must be greater than 0 and less than 1."
...
work_dir="${o}/00-logs_and_reports/logs"
  ...
  conda_activate() {
    ...
    if [ -z "$CONDA_DEFAULT_ENV" ]; then
      "${stage}"_info "${log_mode}" "You didn't activate any conda environment, so HybSuite will help you activate ${conda} environment." 
      current_shell=$(basename "$SHELL")
      set +u
      case "$current_shell" in
        bash)
      ...
      if [ "$CONDA_DEFAULT_ENV" = "${conda}" ]; then
        "${stage}"_info "${log_mode}" "Successfully activating conda environment ${conda}."
        "${stage}"_blank "${log_mode}" ""
      else
        "${stage}"_error "Fail to activate ${conda} conda environment"
      fi
    fi
    if [ "${CONDA_DEFAULT_ENV}" != "${conda}" ]; then
      "${stage}"_info "${log_mode}" "You didn't activate conda environment ${conda}, so HybSuite will help you activate ${conda} environment." 
      current_shell=$(basename "$SHELL")
      set +u
      case "$current_shell" in
        bash)
      ...
      "${stage}"_cmd "${log_mode}" "conda activate ${conda}" 
      if [ "$CONDA_DEFAULT_ENV" = "${conda}" ]; then
        "${stage}"_info "${log_mode}" "Successfully activating ${conda} conda environment" 
        "${stage}"_blank "${log_mode}" ""
      else
        "${stage}"_error "Fail to activate ${conda} conda environment"
      ...
      set -u
      "${stage}"_info "${log_mode}" "You are now in conda environment ${conda}, so HybSuite will skip activating ${conda} conda environment."
      "${stage}"_blank_main ""
    fi
  }
  stage0_blank ""
...
if [ "${skip_stage1}" != "TRUE" ] && [ "${skip_stage12}" != "TRUE" ] && [ "${skip_stage123}" != "TRUE" ] && [ "${skip_stage1234}" != "TRUE" ]; then
  ...
  stage1_cmd() {
    ...
    if [ "${log_mode}" != "simple" ]; then
      echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" | tee -a "${o}/00-logs_and_reports/logs/Stage1_NGS_dataset_construction_${current_time}.log"
    fi
  }
  stage1_blank_main() {
...
if [ "${skip_stage12}" != "TRUE" ] && [ "${skip_stage123}" != "TRUE" ] && [ "${skip_stage1234}" != "TRUE" ]; then
  ...
  stage2_cmd() {
    local log_mode="$1"
    local message="$2"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [INFO] $message" >> "${o}/00-logs_and_reports/logs/HybSuite_cmd_${current_time}.log"
    if [ "${log_mode}" != "simple" ]; then
      echo "[$(date '+%Y-%m-%d %H:%M:%S')] [INFO] $message" | tee -a "${o}/00-logs_and_reports/logs/Stage2_Data_assembly_and_filtered_paralogs_recovering_${current_time}.log"
    fi
  }
  ...
  #################===========================================================================
    stage2_cmd "${log_mode}" "hybpiper paralog_retriever ${eas_dir}/Assembled_data_namelist.txt "-t_${hybpiper_tt}" ${t} --fasta_dir_all ${o}/02-All_paralogs --heatmap_dpi ${hybpiper_heatmap_dpi}"
  ...
  ############################################################################################
  #Stage2-Step3: Filtering paralogs by length, locus and taxon coverage ######################
  ############################################################################################
  #################===========================================================================
    stage2_info_main "Step 3: Filtering paralogs by length, locus and taxon coverage..."
  ...
  #################===========================================================================
    ...
    # 02-Filtering out taxa with low locus coverage and loci with low taxon coverage
    stage2_info_main "02-Filtering out taxa with low locus coverage and loci with low taxon coverage via 'filter_seqs_by_taxon_and_locus_coverage.py'..."
    stage2_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py -i ${o}/02-All_paralogs/03-Filtered_paralogs --min_locus_coverage ${min_locus_coverage} --min_taxon_coverage ${min_taxon_coverage} --removed_taxa_info ${o}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv --removed_loci_info ${o}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv --threads ${nt}"
    stage2_02="python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py \
    -i ${o}/02-All_paralogs/03-Filtered_paralogs \
    --min_locus_coverage ${min_locus_coverage} \
    --min_taxon_coverage ${min_taxon_coverage} \
    --removed_taxa_info ${o}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv \
    --removed_loci_info ${o}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv \
    --threads ${nt}"
    if [ "${log_mode}" != "simple" ]; then
      eval "${stage2_02}"
    else
    ...
    stage2_info "${log_mode}" "${o}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv"
    stage2_info "${log_mode}" "The information of filtered out loci with low taxon coverage has been written to:"
    stage2_info "${log_mode}" "${o}/02-All_paralogs/04-Filtered_paralog_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv"
    stage2_blank_main ""
    ############################################################################################
...
if [ "${skip_stage1234}" != "TRUE" ] && [ "${skip_stage123}" != "TRUE" ]; then
  ...
  stage3_cmd() {
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" >> "${o}/00-logs_and_reports/logs/HybSuite_cmd_${current_time}.log"
    if [ "${log_mode}" != "simple" ]; then
      echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" | tee -a "${o}/00-logs_and_reports/logs/Stage3_Orthology_inference_${current_time}.log"
    fi
  }
  stage3_blank_main() {
  ...
  if [ "${HRS}" = "TRUE" ]; then
    ...
    # 05-Filtering all HRS sequences by length, locus and taxon coverage
    #################===========================================================================
    stage3_info_main "Step 2: Filtering all HRS sequences by length, locus and taxon coverage ..." 
    ...
    # 02-Filtering out taxa with low locus coverage and loci with low taxon coverage
    stage3_info_main "02-Filtering out taxa with low locus coverage and loci with low taxon coverage..."
    stage3_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py -i ${o}/03-Orthology_inference/HRS/03-Filtered_HRS_sequences/ --min_locus_coverage ${min_locus_coverage} --min_taxon_coverage ${min_taxon_coverage} --removed_taxa_info ${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv --removed_loci_info ${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv --threads ${nt}"
    stage3_HRS_02="python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py \
    -i ${o}/03-Orthology_inference/HRS/03-Filtered_HRS_sequences/ \
    --min_locus_coverage ${min_locus_coverage} \
    --min_taxon_coverage ${min_taxon_coverage} \
    --removed_taxa_info ${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv \
    --removed_loci_info ${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv \
    --threads ${nt}"
    if [ "${log_mode}" != "simple" ]; then
      eval "${stage3_HRS_02}"
    else
    ...
    if [ -s "${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv" ]; then
      stage3_info "${log_mode}" "The information of filtered out taxa with low locus coverage has been written to:"
      stage3_info "${log_mode}" "${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv"
    else
      stage3_error "Fail to filter out taxa with low locus coverage."
    fi
    if [ -s "${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv" ]; then
      stage3_info "${log_mode}" "The information of filtered out loci with low taxon coverage has been written to:"
      stage3_info "${log_mode}" "${o}/03-Orthology_inference/HRS/04-Filtered_HRS_sequences_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv"
    else
      stage3_error "Fail to filter out loci with low taxon coverage."
    fi
    stage3_info_main "Finish"
    stage3_blank "${log_mode}" ""
    ...
    satge3_HRS_04="python ${script_dir}/plot_recovery_heatmap.py \
    -i ${o}/03-Orthology_inference/HRS/03-Filtered_HRS_sequences/ \
    -r ${t} \
  ...
  if [ "${RLWP}" = "TRUE" ]; then
    ...
    # 03-Filtering out RLWP sequences with low length, taxon and locus coverage
    stage3_info_main "Step 3: Filtering out RLWP sequences with low length, taxon and locus coverage..."
    ...
    # 04-Filtering out taxa with low locus coverage and loci with low taxon coverage
    stage3_info_main "02-Filtering out taxa with low taxon and loci with low taxon coverage ..."
    stage3_cmd "${log_mode}" "python "${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py" -i "${o}/03-Orthology_inference/RLWP/03-Filtered_RLWP_sequences/" --min_locus_coverage "${min_locus_coverage}" --min_taxon_coverage "${min_taxon_coverage}" --removed_taxa_info "${o}/03-Orthology_inference/RLWP/04-Filtered_RLWP_sequences_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv" --removed_loci_info "${o}/03-Orthology_inference/RLWP/04-Filtered_RLWP_sequences_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv" --threads "${nt}""
    stage3_RLWP_3="python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py \
    -i ${o}/03-Orthology_inference/RLWP/03-Filtered_RLWP_sequences/ \
    --min_locus_coverage ${min_locus_coverage} \
    --min_taxon_coverage ${min_taxon_coverage} \
    --removed_taxa_info ${o}/03-Orthology_inference/RLWP/04-Filtered_RLWP_sequences_reports_and_heatmap/Filtered_out_taxa_with_low_locus_coverage_info.tsv \
    --removed_loci_info ${o}/03-Orthology_inference/RLWP/04-Filtered_RLWP_sequences_reports_and_heatmap/Filtered_out_loci_with_low_taxon_coverage_info.tsv \
    --threads ${nt}"
    if [ "${log_mode}" != "simple" ]; then
      eval "${stage3_RLWP_3}"
    else
...
if [ "${skip_stage1234}" != "TRUE" ]; then
  ...
  stage4_cmd() {
    ...
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" >> "${o}/00-logs_and_reports/logs/HybSuite_cmd_${current_time}.log"
    if [ "$log_mode" != "simple" ]; then
      echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" | tee -a "${o}/00-logs_and_reports/logs/Stage4_Sequence_alignment_trimming_and_supermatrix_construction_${current_time}.log"
    fi
  }
  stage4_blank() {
    local log_mode="$1"
  ...
  if [ "${LS}" = "TRUE" ] || [ "${MI}" = "TRUE" ] || [ "${MO}" = "TRUE" ] || [ "${RT}" = "TRUE" ] || [ "${one_to_one}" = "TRUE" ] && [ "${run_phylopypruner}" = "TRUE" ]; then
    ...
    stage4_ortho_phylopypruner () {
      ...
      stage4_info_main "02-Removing ${ortho_method} orthogroups with <${min_taxon_coverage} sample coverage ..."
      stage4_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py -i ${o}/04-Alignments/${ortho_method}/ --min_taxon_coverage ${min_taxon_coverage} --removed_taxa_info ${o}/04-Alignments/${ortho_method}/Removed_alignments_with_low_sample_coverage.txt -t ${nt}"
      stage4_02="python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py -i ${o}/04-Alignments/${ortho_method}/ --min_taxon_coverage ${min_taxon_coverage} --removed_taxa_info ${o}/04-Alignments/${ortho_method}/Removed_alignments_with_low_sample_coverage.txt -t ${nt}"
      if [ "${log_mode}" != "simple" ]; then
        eval "${stage4_02}"
      else
    ...
    if [ "${LS}" = "TRUE" ]; then
      stage4_ortho_phylopypruner "LS" "Output_LS" "${prefix}" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${MI}" = "TRUE" ]; then
      stage4_ortho_phylopypruner "MI" "Output_MI" "${prefix}" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${MO}" = "TRUE" ]; then
      stage4_ortho_phylopypruner "MO" "Output_MO" "${prefix}" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${RT}" = "TRUE" ]; then
      stage4_ortho_phylopypruner "RT" "Output_RT" "${prefix}" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${one_to_one}" = "TRUE" ]; then
      stage4_ortho_phylopypruner "1to1" "Output_1to1" "${prefix}" "${nt}" "${min_taxon_coverage}"
    fi
  fi
  stage4_blank_main ""
  ...
  if [ "${MO}" = "TRUE" ] || [ "${MI}" = "TRUE" ] || [ "${RT}" = "TRUE" ] || [ "${one_to_one}" = "TRUE" ] && [ "${run_paragone}" = "TRUE" ]; then
    ...
    stage4_ortho_paragone () {
      ...
      stage4_info_main "02-Removing ${ortho_method} orthogroups with <${min_taxon_coverage} sample coverage ..."
      stage4_cmd "${log_mode}" "python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py -i ${o}/04-Alignments/${ortho_method}/ -c ${min_taxon_coverage} --removed_taxa_info ${o}/04-Alignments/${ortho_method}/Removed_alignments_with_low_sample_coverage.txt -t ${nt}"
      stage4_02="python ${script_dir}/filter_seqs_by_taxon_and_locus_coverage.py -i ${o}/04-Alignments/${ortho_method}/ --min_taxon_coverage ${min_taxon_coverage} --removed_taxa_info ${o}/04-Alignments/${ortho_method}/Removed_alignments_with_low_sample_coverage.txt -t ${nt}"
      if [ "${log_mode}" != "simple" ]; then
        eval "${stage4_02}"
      else
    ...
    if [ "${MO}" = "TRUE" ]; then
      stage4_ortho_paragone "MO" "26_MO_final_alignments_trimmed" "MO" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${MI}" = "TRUE" ]; then
      stage4_ortho_paragone "MI" "27_MI_final_alignments_trimmed" "MI" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${RT}" = "TRUE" ]; then
      stage4_ortho_paragone "RT" "28_RT_final_alignments_trimmed" "RT" "${nt}" "${min_taxon_coverage}"
    fi
    if [ "${one_to_one}" = "TRUE" ]; then
      stage4_ortho_paragone "1to1" "HybSuite_1to1_final_alignments_trimmed" "1to1" "${nt}" "${min_taxon_coverage}"
    fi
  fi
  stage4_blank "${log_mode}" ""
  ############################################################################################
...
stage5_cmd() {
  ...
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" >> "${o}/00-logs_and_reports/logs/HybSuite_cmd_${current_time}.log"
  if [ "${log_mode}" != "simple" ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [CMD] $message" | tee -a "${o}/00-logs_and_reports/logs/Stage5_Phylogenetic_tree_inference_${current_time}.log"
  fi
}
stage5_blank() {
  local log_mode="$1"
...
#Preparation
if [ "${run_astral}" = "TRUE" ] || [ "${run_wastral}" = "TRUE" ]; then
  stage5_info_main "Optional step: Constructing coalescent-based trees (ASTRAL or wASTRAL) for ${ortho_method} alignments."
  run_raxml_sg() {
    local ortho_method="$1"
    local Genename="$2"
  ...
  run_wastral() {
    ...
    mkdir -p ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees
    cd ${wastral_dir}
    define_threads "wastral" "stage5_logfile"
    # if the number of species is less than 2000, run wastral
    if [ $(wc -l < ${o}/08-Coalescent-based_trees/${ortho_method}/Final_genes_for_coalscent_list.txt) -lt 2000 ]; then
      bin/wastral --mode ${wastral_mode} -t ${nt_wastral} \
      -r ${wastral_R} -s ${wastral_S} \
      -o ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre \
      ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre 2> ${o}/00-logs_and_reports/logs/wASTRAL_${prefix}_${ortho_method}.log
      stage5_cmd "${log_mode}" "bin/wastral --mode ${wastral_mode} -t ${nt_wastral} -r ${wastral_R} -s ${wastral_S} -o ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre 2> ${o}/00-logs_and_reports/logs/wASTRAL_${prefix}_${ortho_method}.log"
      bin/wastral -S ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre > ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_bootstrap_${prefix}_${ortho_method}.tre
      stage5_cmd "${log_mode}" "bin/wastral -S ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre > ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_bootstrap_${prefix}_${ortho_method}.tre"
    # if the number of species is more than 2000, run wastral
    else
      bin/wastral_precise --mode ${wastral_mode} -t ${nt_wastral} \
      -r ${wastral_R} -s ${wastral_S} \
      -o "${o}/06-Coalescent-based_trees/${ortho_mode}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_mode}.tre" \
      ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre 2> ${o}/00-logs_and_reports/logs/wASTRAL_${prefix}_${ortho_method}.log
      stage5_cmd "${log_mode}" "bin/wastral_precise --mode ${wastral_mode} -t ${nt_wastral} -r ${wastral_R} -s ${wastral_S} -o ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre 2> ${o}/00-logs_and_reports/logs/wASTRAL_${prefix}_${ortho_method}.log"
      bin/wastral_precise -S ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre > ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_bootstrap_${prefix}_${ortho_method}.tre
      stage5_cmd "${log_mode}" "bin/wastral_precise -S ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre > ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_bootstrap_${prefix}_${ortho_method}.tre"
    fi
      
    if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_bootstrap_${prefix}_${ortho_method}.tre" ]; then
  ...
  calculate_branch_length() {
    ...
    # Delete existed RAxML files
      if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/RAxML_info.ASTRAL_${prefix}_${ortho_method}_bl.tre" ]; then
        rm ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/RAxML*
      fi
    # run RAxML
      stage5_cmd "${log_mode}" "raxmlHPC -f e -t ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_${prefix}_${ortho_method}.tre -m GTRGAMMA -s ${o}/05-Supermatrix/${ortho_method}/${prefix}_${ortho_method}.fasta -n ASTRAL_${prefix}_${ortho_method}_bl.tre -w ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees -N 100"
      raxmlHPC -f e -t ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_${prefix}_${ortho_method}.tre -m GTRGAMMA \
      -s ${o}/05-Supermatrix/${ortho_method}/${prefix}_${ortho_method}.fasta \
      -n ASTRAL_${prefix}_${ortho_method}_bl.tre \
      -w ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees \
      -N 100 > /dev/null 2>&1
      cd ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees

    # reroot the ASTRAL tree
      counter=1
      while IFS= read -r line || [ -n "$line" ]; do
        eval "og${counter}=\"${line}\""
        counter=$((counter + 1))
      done < ${i}/Outgroup.txt
      cmd="pxrr -s -t ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/RAxML_result.ASTRAL_${prefix}_${ortho_method}_bl.tre -g "
      x=1
      og_params=""
      while [ $x -lt $counter ]; do
        var_name="og$x"
      ...
      cmd="${cmd}${og_params}"
	  stage5_cmd "${log_mode}" "${cmd} | nw_order -c n - > ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre"
	  eval "${cmd} | nw_order -c n - > ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre"
	  if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/RAxML_result.ASTRAL_${prefix}_${ortho_method}_bl.tre" ]; then
        stage5_info_main "Finish"
      else
        stage5_error "Fail to calculate the branch length via RAxML."
        stage5_blank_main ""
      fi
    fi
    # wASTRAL ############
    if [ "${run_wastral}" = "TRUE" ]; then
    # Delete existed RAxML files
      if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/RAxML_info.wASTRAL_${prefix}_${ortho_method}_bl.tre" ]; then
        rm ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/RAxML*
      fi
    # run RAxML
      raxmlHPC -f e -t ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre -m GTRGAMMA \
      -s ${o}/05-Supermatrix/${ortho_method}/${prefix}_${ortho_method}.fasta \
      -n wASTRAL_${prefix}_${ortho_method}_bl.tre \
      -w ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees \
      -N 100
      stage5_cmd "${log_mode}" "raxmlHPC -f e -t ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_${prefix}_${ortho_method}.tre -m GTRGAMMA -s ${o}/05-Supermatrix/${ortho_method}/${prefix}_${ortho_method}.fasta -n wASTRAL_${prefix}_${ortho_method}_bl.tre -w ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees -N 100"
      cd ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees
    # reroot the wASTRAL tree
      counter=1
      while IFS= read -r line || [ -n "$line" ]; do
        eval "og${counter}=\"${line}\""
        counter=$((counter + 1))
      done < ${i}/Outgroup.txt
      cmd="pxrr -s -t ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/RAxML_result.wASTRAL_${prefix}_${ortho_method}_bl.tre -g "
      x=1
      og_params=""
      while [ $x -lt $counter ]; do
        var_name="og$x"
      ...
      cmd="${cmd}${og_params}"
	  stage5_cmd "${log_mode}" "${cmd} | nw_order -c n - > ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre"
	  eval "${cmd} | nw_order -c n - > ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre"
      if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/RAxML_result.wASTRAL_${prefix}_${ortho_method}_bl.tre" ]; then
        stage5_info_main "Finish"
      else
        stage5_error "Fail to calculate the branch length via RAxML."
        stage5_blank_main ""
  ...
  run_phyparts_piecharts() {
    ...
    mkdir -p ${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts
    cd ${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts
    # ASTRAL ############
    if [ "${run_astral}" = "TRUE" ]; then
      stage5_info_main "Running PhyPartsPieCharts for ${ortho_method} coalescent-based tree (ASTRAL) ..."
      stage5_cmd "${log_mode}" "java -jar ${script_dir}/../dependencies/phypartspiecharts/target/phyparts-0.0.1-SNAPSHOT-jar-with-dependencies.jar -a 1 -v -d ${o}/08-Coalescent-based_trees/${ortho_method}/02-rerooted_gene_trees -m ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre -o ${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts/ASTRAL_PhyParts"
      ...
      -m ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre \
      -o ${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts/ASTRAL_PhyParts &> /dev/null
      ...
      ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre \
      ASTRAL_PhyParts ${phyparts_number} \
      --svg_name ASTRAL_PhyPartsPieCharts_${prefix}_${ortho_method}.svg \
      --to_csv
    
      stage5_cmd "${log_mode}" "python3 ${script_dir}/../dependencies/phypartspiecharts/phypartspiechart.py ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre ASTRAL_PhyParts ${phyparts_number} --svg_name PhypartsPiecharts_${prefix}_${ortho_method}.svg --to_csv"
      if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts/ASTRAL_PhyPartsPieCharts_${prefix}_${ortho_method}.svg" ]; then
        stage5_info_main "Finish"
      else
    ...
    if [ "${run_wastral}" = "TRUE" ]; then
      ...
      stage5_cmd "${log_mode}" "java -jar ${script_dir}/../dependencies/phypartspiecharts/target/phyparts-0.0.1-SNAPSHOT-jar-with-dependencies.jar -a 1 -v -d ${o}/08-Coalescent-based_trees/${ortho_method}/02-rerooted_gene_trees -m ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre -o ${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts/wASTRAL_PhyParts"
      java -jar ${script_dir}/../dependencies/phypartspiecharts/target/phyparts-0.0.1-SNAPSHOT-jar-with-dependencies.jar \
      -a 1 -v -d ${o}/08-Coalescent-based_trees/${ortho_method}/02-rerooted_gene_trees \
      -m ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre \
      -o ${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts/wASTRAL_PhyParts &> /dev/null
            
      phyparts_number=$(find ${o}/08-Coalescent-based_trees/${ortho_method}/02-rerooted_gene_trees -type f -name "*_rr.tre" | wc -l)
      python3 ${script_dir}/../dependencies/phypartspiecharts/phypartspiechart.py \
      ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre \
      wASTRAL_PhyParts ${phyparts_number} \
      --svg_name wASTRAL_PhyPartsPieCharts_${prefix}_${ortho_method}.svg \
      --to_csv
  
      stage5_cmd "${log_mode}" "python3 ${script_dir}/../dependencies/phypartspiecharts/phypartspiechart.py ${o}/08-Coalescent-based_trees/${ortho_method}/04-wASTRAL_final_trees/wASTRAL_final_ordered_rr_${prefix}_${ortho_method}.tre wASTRAL_PhyParts ${phyparts_number} --svg_name PhypartsPiecharts_${prefix}_${ortho_method}.svg --to_csv"
      if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/05-PhyParts_PieCharts/wASTRAL_PhyPartsPieCharts_${prefix}_${ortho_method}.svg" ]; then
        stage5_info_main "Finish"
      else
        stage5_error "Fail to run phypartspiecharts.py."
        stage5_blank_main ""
  ...
  run_coalescent_trees() {
    ...
    > "${o}/08-Coalescent-based_trees/${ortho_method}/Final_alignments_with_more_than_5_seqs_list.txt"
    cd ${o}/04-Alignments/${ortho_method}/
    ...
    for Gene in *.trimmed.aln.fasta; do
      if [ $(grep -c '^>' "${Gene}") -lt 5 ]; then
        ...
        echo "$Gene" >> "${o}/08-Coalescent-based_trees/${ortho_method}/Final_alignments_with_more_than_5_seqs_list.txt"
      fi
    done
    
    total_sps=$(awk 'NF' "${o}/08-Coalescent-based_trees/${ortho_method}/Final_alignments_with_more_than_5_seqs_list.txt" | wc -l)
    init_parallel_env "$work_dir" "$total_sps" "$process" "${o}/08-Coalescent-based_trees/${ortho_method}/Final_alignments_with_more_than_5_seqs_list.txt" || exit 1
    ...
    done < "${o}/08-Coalescent-based_trees/${ortho_method}/Final_alignments_with_more_than_5_seqs_list.txt"
    wait
    echo
    if [ "${log_mode}" != "simple" ]; then
    ...
    cat ${o}/08-Coalescent-based_trees/${ortho_method}/02-rerooted_gene_trees/*_rr.tre >> ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre
    ...
    if [ "${run_astral}" = "TRUE" ]; then
      ...
      mkdir -p ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees
      java -jar ${script_dir}/../dependencies/ASTRAL-master/Astral/astral.5.7.8.jar \
      -i ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre \
      -o ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_${prefix}_${ortho_method}.tre 2> ${o}/00-logs_and_reports/logs/ASTRAL_${prefix}_${ortho_method}.log
      stage5_cmd "${log_mode}" "java -jar ${script_dir}/../dependencies/ASTRAL-master/Astral/astral.5.7.8.jar -i   -i ${o}/08-Coalescent-based_trees/${ortho_method}/03-combined_gene_trees/${prefix}_${ortho_method}_combined.tre -o ${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_${prefix}_${ortho_method}.tre 2> ${o}/00-logs_and_reports/logs/${prefix}_${ortho_method}_ASTRAL.log"
      if [ -s "${o}/08-Coalescent-based_trees/${ortho_method}/04-ASTRAL_final_trees/ASTRAL_${prefix}_${ortho_method}.tre" ]; then
    ...
    ############################################################
    stage5_blank "${log_mode}" ""
    stage5_info_main "Step5: Caculate the branch length via RAxMl"
    calculate_branch_length "${ortho_method}"
