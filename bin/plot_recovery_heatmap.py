...
def calculate_seq_lengths(input_dir, species_list_file, output_species_list, output_file, ref_file, threads=1, use_max=False):
    ...
    final_df = pd.concat([
        pd.DataFrame({'Taxon/Locus': ['MeanLength(Targetfile)', 'MaxLength(Targetfile)'] + species}),
        pd.DataFrame([ref_lengths_list, max_lengths_list] + df.values.tolist(), 
                    columns=df.columns)
    ], axis=1)
    
    numeric_columns = final_df.columns.difference(['Taxon/Locus'])
    ...
    if not use_max:
        return pd.concat([
            pd.DataFrame({'Taxon/Locus': ['MeanLength(Targetfile)'] + species}),
...
def create_heatmap(df, args):
    ...
    numeric_columns = df.columns.difference(['Taxon/Locus'])
    ...
    recovery_rates['Taxon/Locus'] = df['Taxon/Locus']
    
    color_rates = pd.DataFrame(index=df.index, columns=df.columns)
    color_rates['Taxon/Locus'] = df['Taxon/Locus']
    ...
    plot_data = color_rates.set_index('Taxon/Locus')
    ...
    if args.show_values:
        annot = recovery_rates.set_index('Taxon/Locus')
...
def standalone():
    ...
    parser.add_argument('--ylabel',
                        help='Y-axis label (default: "Taxon")')
